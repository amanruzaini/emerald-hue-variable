...
import { variablesStore, VariableType, VariablesStore, PublishedVariable } from './variables-utils';
import { generateVariableSuggestions, VariableSuggestion } from './variable-suggestions';
import { figmaColorToRgb255 } from './value-comparison';

// Existing types from the code.ts file that our integration needs to work with
// Note that these are the external types used in the main code
interface ExistingVariableMapEntry {
  variable: ExistingVariableDataWithValues | null;
  aliases: string[];
}

interface ExistingVariableDataWithValues {
  id: string;
  name: string;
  collectionName: string;
  collectionId: string;
  resolvedType: string; // This is the key difference - we treat it as string to be more flexible
  value: any;
  isAlias: boolean;
  parentId?: string;
}

// This is what our suggestion looks like from the existing code
...
function convertToExistingSuggestionFormat(suggestion: VariableSuggestion): ExistingSuggestion {
  return {
 ...
 */
function adaptVariableMap(externalMap: Record<string, ExistingVariableMapEntry>): Record<string, PublishedVariable> {
  const adaptedMap: Record<string, PublishedVariable> = {};
 ...
 */
function mapResolvedType(resolvedType: string): VariableType {
  switch (resolvedType) {
    case 'COLOR':
      return 'COLOR';
    case 'NUMBER':
 ...
 */
export function initializeVariableStore(variableMap: Record<string, ExistingVariableMapEntry>): void {
 ...
 */
export function findBestVariableSuggestion(
  propertyName: string,
  value: any
 ...
 */
export function findClosestColorVariableCompat(
