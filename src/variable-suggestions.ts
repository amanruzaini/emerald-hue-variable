...
import { PublishedVariable, VariableType, variablesStore } from './variables-utils';
import { getPropertyExpectedVariableType } from './property-mapping';
import { 
  FigmaColor, RGB, figmaColorToRgb255, getColorDifference, 
  getNumberDifference, calculateColorConfidence, calculateNumberConfidence,
  COLOR_PROXIMITY_THRESHOLD, NUMBER_PROXIMITY_THRESHOLD, compareColors, compareNumbers
} from './value-comparison';

// Suggestion types
export type SuggestionType = 'PUBLISHED_DIRECT' | 'PUBLISHED_PROXIMITY';

// Suggestion result
export interface VariableSuggestion {
  type: SuggestionType;
  propertyName: string;
  suggestedVariable: PublishedVariable;
...
function findClosestColorVariable(
  color: FigmaColor, 
  colorVariables: PublishedVariable[]
): VariableSuggestion | null {
  ...
  let bestSuggestion: VariableSuggestion | null = null;
 ...
 */
function findClosestNumberVariable(
  number: number,
  numberVariables: PublishedVariable[]
): VariableSuggestion | null {
